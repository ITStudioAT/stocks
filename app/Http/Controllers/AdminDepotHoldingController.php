...
class AdminDepotHoldingController extends Controller
{
    ...
    public function exportPdf(): Response
    {
        ...
        $pdf = Pdf::loadView('pdf.watchlist', [
            'holdings' => $holdings,
            'depot' => $this->activeDepotPayload(),
            'generatedAt' => $generatedAt,
        ])->setPaper('a4', 'portrait');

        return $pdf->download('watch-list-'.$generatedAt->format('Y-m-d-His').'.pdf');
    }
    ...
    {
        $dispatchedRefresh = $this->priceRefreshScheduler->dispatchWatchlist();
