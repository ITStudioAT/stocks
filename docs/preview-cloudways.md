...
Der lokale Helfer `scripts/preview-snapshot-upload.ps1 -BundleDirectory PRIVATE_BUNDLE -ExpectedManifestSha256 TRUSTED_MANIFEST_SHA256` führt diesen Ablauf einschließlich Upload aus. Er prüft vorher die exakt 13 Paketdateien gegen das separat bestätigte Manifest, bindet das Ziel an den installierten Commit und verlangt die bereits bekannte SSH-Serveridentität. `-VerifyOnly` prüft das Paket ohne Netzwerkzugriff. Die Anmeldung erfolgt direkt durch OpenSSH im Nutzerterminal; das Skript liest oder speichert kein Passwort. Jeder Versuch erhält zusätzlich zum Snapshot-Nonce eine zufällige Kennung im privaten Zielpfad. Frühere, möglicherweise nur teilweise hochgeladene Verzeichnisse werden weder überschrieben noch entfernt. Vor dem Anlegen stoppt der Helfer bei einer vorhandenen Wartungsdatei oder einem offenen Dateiaustausch, damit ein früherer Import zuerst anhand seiner eigenen Belege geklärt wird. Bei Abbruch bleiben Paket und Wiederherstellungsbelege erhalten; der Helfer nennt den konkreten Restore-Aufruf. Die lokalen Tests prüfen zusätzlich getrennte Wiederholungsversuche, manipulierte Dateien, falsche Zielidentität und unzulässige Manifestpfade. MySQL prüft Wiederherstellung und Freigabe mit jeweils neuer CLI-Instanz sowie begrenzten Speicher beim Sperren großer vorhandener Datenbestände.
